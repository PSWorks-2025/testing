...
const AssessmentIdPage = async ({
  ...
  try {
    ...
    console.error(error);
    const mockAssessment = assessments.find(
      (x) => x.id === params.assessmentId
    );
    if (!mockAssessment) {
      throw new Error('Assessment not found in mock data');
    }
    assessment = mockAssessment as AssessmentExtended;
  }
  if (!assessment || !mode) {
    return notFound();
  }
